...
function QueueAnswer({ req, onAnswered, language = "en" }) {
  ...
  return (
    <div style={{ marginTop: 10, display: "flex", gap: 8 }}>
      <input
        ...
        placeholder={t(language, "it.replyPlaceholder")}
...
export default function ITSupport({ language = "en" }) {
  ...
  const AnnouncementsList = (
    announcements.length === 0 ? (
      <p style={{ fontSize: 12.5, color: "var(--text-muted)", marginBottom: 28 }}>{t(language, "it.noUpdates")}</p>
    ) : (
    ...
    <div style={{ height: "100%", overflowY: "auto" }} className="chat-scroll">
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "24px 20px 40px" }}>
        <h1 style={{ fontSize: 20, fontWeight: 600, color: "var(--text-primary)", margin: 0, display: "flex", alignItems: "center", gap: 10 }}>
          <Headset size={20} strokeWidth={1.75} color="var(--accent)" /> {isIT ? t(language, "it.consoleTitle") : t(language, "it.supportTitle")}
        </h1>
        <p style={{ fontSize: 13, color: "var(--text-secondary)", margin: "4px 0 24px" }}>
          {isIT ? t(language, "it.consoleSubtitle") : t(language, "it.supportSubtitle")}
        </p>
        ...
        {isIT ? (
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 12, marginBottom: 28 }}>
              <Stat icon={Inbox} value={itQ.count_open} label={t(language, "it.openQuestions")} color="#EF4444" />
              <Stat icon={CheckCircle2} value={itQ.answered.length} label={t(language, "it.answered")} color="#16A34A" />
              <Stat icon={Megaphone} value={announcements.length} label={t(language, "it.activeUpdates")} />
            </div>

            {/* Queue to answer */}
            <SectionTitle>
              <HelpCircle size={16} color="var(--accent)" /> {t(language, "it.questionsToAnswer")}
            ...
            {itQ.open.length === 0 ? (
              <p style={{ fontSize: 12.5, color: "var(--text-muted)", marginBottom: 28 }}>{t(language, "it.allCaughtUp")}</p>
            ) : (
                    ...
                    >
                      ...
                      {isOpen && (
                        <div onClick={(e) => e.stopPropagation()}>
                          <QueueAnswer req={r} language={language} onAnswered={() => { loadITQ(); setExpandedId(null); }} />
                        </div>
                      )}
                    </div>
                  );
            ...
            <SectionTitle action={
              <button onClick={() => setComposing(true)} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "6px 12px", borderRadius: 8, border: "1px solid var(--accent)", background: "var(--accent-tint)", color: "var(--accent)", fontSize: 12.5, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}>
                <Plus size={14} /> {t(language, "it.postUpdate")}
              </button>
            }>
              <Megaphone size={16} color="var(--accent)" /> {t(language, "it.broadcast")}
            </SectionTitle>
            {AnnouncementsList}
            ...
            {itQ.answered.length > 0 && (
              <>
                <h2 style={{ fontSize: 14, fontWeight: 600, color: "var(--text-primary)", margin: "0 0 10px" }}>{t(language, "it.recentlyAnswered")}</h2>
            ...
            <h2 style={{ fontSize: 14, fontWeight: 600, color: "var(--text-primary)", margin: "0 0 12px" }}>{t(language, "it.yourTeam")}</h2>
              ...
              {team.map((m, i) => (
                <div key={m.id} style={{ border: "1px solid var(--border-color)", borderRadius: 12, padding: 14, backgroundColor: "var(--bg-card)", display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 13.5, fontWeight: 600, color: "var(--text-primary)" }}>{m.name}{m.name === me ? ` (${t(language, "common.you")})` : ""}</div>
          ...
          <>
            <h2 style={{ fontSize: 14, fontWeight: 600, color: "var(--text-primary)", margin: "0 0 12px" }}>{t(language, "it.getHelp")}</h2>
                ...
                <div key={m.id} style={{ border: "1px solid var(--border-color)", borderRadius: 12, padding: 16, backgroundColor: "var(--bg-card)", display: "flex", flexDirection: "column", gap: 12 }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={() => setAsk({ member: m.name, mode: "ask" })} style={{ flex: 1, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "8px", borderRadius: 8, border: "none", backgroundColor: "var(--accent)", color: "#fff", fontSize: 12.5, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}>
                      <MessageSquare size={14} /> {t(language, "common.ask")}
                    </button>
                    <button onClick={() => setAsk({ member: m.name, mode: "meet" })} style={{ flex: 1, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "8px", borderRadius: 8, border: "1px solid var(--border-color)", background: "transparent", color: "var(--text-primary)", fontSize: 12.5, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}>
                      <CalendarPlus size={14} /> {t(language, "common.officeHours")}
                    </button>
                  </div>
                </div>
              ))}
            ...
            <h2 style={{ fontSize: 14, fontWeight: 600, color: "var(--text-primary)", margin: "0 0 10px", display: "flex", alignItems: "center", gap: 8 }}>
              <Megaphone size={16} color="var(--accent)" /> {t(language, "it.updatesFromIt")}
            </h2>
            {AnnouncementsList}
